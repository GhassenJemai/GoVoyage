...
class VolController extends Controller
{
    ...
    {
        $vol = new Vol();
        $form = $this->createForm('GoVoyageBundle\Form\VolType', $vol);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('vol_show', array('numticket' => $vol->getNumticket()));
        }
    ...
    {
        $deleteForm = $this->createDeleteForm($vol);
        $editForm = $this->createForm('GoVoyageBundle\Form\VolType', $vol);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('vol_edit', array('numticket' => $vol->getNumticket()));
        }
    ...
    private function createDeleteForm(Vol $vol)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('vol_delete', array('numticket' => $vol->getNumticket())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
